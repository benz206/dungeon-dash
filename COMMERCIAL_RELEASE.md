...
- 93 Unity tests and 10 Node market tests pass. Unity tests ran in an isolated
...
- The credits panel was compiled and visually verified at both sizes; bundled
  third-party notices match the source text. The subsequent 93-test suite includes
  the account-deletion and optional online-connection changes. An earlier credits
  build needed its isolated editor terminated after successful build completion;
  the final Mac build and the account-panel QA runs exited cleanly.
...
The detailed [asset audit](Docs/Release/AssetProvenance.md) and hash inventories
account for 398 media files and 61 resolved packages. All 370 imported sprites
match the original Godot source, all eight generated tiles match regenerated
pixels, and all 17 audio files match regenerated bytes. Credits are available
from title and pause; available package and font notices are bundled in builds.
...
art. The inventory is captured; final author-archive provenance and engine-license
sign-off remain part of submission preparation.
...
- [English store listing and IAP metadata](Docs/Release/StoreListing.md) — field
  lengths checked; native iPhone screenshots and App Store Connect entry pending.
- [Privacy implementation audit](Docs/Release/PrivacyAudit.md) — source-backed data
  flows, SDK manifest inventory, and specific remaining implementation work.
- [Unpublished privacy policy draft](Docs/Release/PrivacyPolicyDraft.md) and
  [support page draft](Docs/Release/SupportDraft.md) — require real publisher,
  contact, hosting, retention, and deletion details before publication.

New players choose **Online Data > Connect Online** before an anonymous UGS
account is created. Existing online accounts retain their connection. The in-app
deletion flow and server cleanup code now exist and pass automated tests, including
restart, serialization, stale requests, and concurrent purchases. See
[deletion deployment and verification](Docs/Release/OnlineAccountDeletion.md).
The changed backend has not been deployed, and end-to-end deletion in the game
remains unverified. A disposable live Authentication account was created and
removed to verify Unity's error responses; no existing players or market records
were touched. Lost confirmation after credentials become invalid still requires
an operational support route. That route, backend verification, retention review,
and the hosted in-app privacy link remain release blockers.
